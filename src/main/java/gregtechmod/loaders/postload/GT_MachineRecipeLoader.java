...
//        RecipeMaps.CANINNING.factory().EUt(1).duration(50).inputs(GT_OreDictUnificator.get(OrePrefixes.cell, Materials.BioFuel, 1L), GT_Items.Fuel_Can_Plastic_Empty.get(1L)).outputs(GT_ModHandler.setFuelValue(GT_Items.Fuel_Can_Plastic_Filled.get(1L), (short)(Materials.BioFuel.mFuelPower * 400)), GT_Items.Cell_Empty.get(1L)).buildAndRegister();
        ...
        RecipeMaps.FUSION_REACTOR.factory().EUt(32768).startEU(100000000).duration(512).inputs(GT_Utility.copy(Materials.Beryllium.mSolid, 1000), GT_Utility.copy(Materials.Tungsten.mSolid, 1000)).output(GT_OreDictUnificator.get(OrePrefixes.dust, Materials.Cooperite, 1L)).buildAndRegister();
        ...
        RecipeMaps.ASSEMBLING.factory().EUt( 4).duration(3200).input(OrePrefixes.plate, Materials.Cooperite, 1).input(GT_Items.Circuit_Advanced.get(1L)).output(GT_Items.Circuit_Board_Elite.get(1L)).buildAndRegister();
