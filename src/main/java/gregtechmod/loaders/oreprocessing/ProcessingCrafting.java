		...
		for (OreDictEntry entry : entries) {
			if (this.isExecutable(aPrefix, this.getMaterial(aPrefix, entry))) {
				if (entry.oreDictName.equals(GT_OreDictNames.craftingRedstoneTorch.toString())) {
					...
					RecipeMaps.ASSEMBLING.factory().EUt(4).duration(3200).input(RecipeEntry.fromStacks(entry.ores, Match.DAMAGE)).input(OrePrefixes.gearGt, Materials.TungstenSteel, 1).output(GregTech_API.getGregTechComponent(25, 1)).buildAndRegister();
