...
public class GT_MetaTileEntity_Boiler_Steel extends GT_MetaTileEntity_BasicTank {
	...
	@Override public void onRightclick(EntityPlayer aPlayer) {
		if (aPlayer != null) {
			if (GT_Utility.areStacksEqual(aPlayer.getCurrentEquippedItem(), new ItemStack(Items.water_bucket, 1))) {
				...
				getBaseMetaTileEntity().openGUI(aPlayer, 171);
			}
		}
	}
    ...
    public void onPostTick() {
    	if (getBaseMetaTileEntity().isServerSide()) {
    		...
    		if (getBaseMetaTileEntity().getTimer()%10==0) {
	    		if (mTemperature > 100) {
	    			if (mFluid[0] == null || !GT_ModHandler.isWater(mFluid[0]) || mFluid[0].amount <= 0) {
	    				...
	    				mFluid[0].amount--;
		    			if (mSteam == null) {
		    				FluidStack steam = GT_ModHandler.getIC2Steam(150);
		    				if (steam == null) steam = GT_ModHandler.getSteam(150);
		    				mSteam = steam;
		    			} else {
    		...
    		if (mTemperature < 1000 && mProcessingEnergy > 0 && getBaseMetaTileEntity().getTimer()%12==0) {
    			mProcessingEnergy-=2;
    			mTemperature++;
    		}
	...
	public int getTextureIndex(byte aSide, byte aFacing, boolean aActive, boolean aRedstone) {
    	if (aSide == 0) return 351;
    	if (aSide == 1) return 350;
    	if (aSide == aFacing) return aActive?353:352;
    	return 349;
	}
